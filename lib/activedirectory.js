    ...
    async.each(chunks, function getUsersForGroup_ChunkItem(members, asyncChunkCallback) {
      ...
      search.call(self, localOpts, function onSearch(err, members) {
        ...
        async.forEach(members, function(member, asyncCallback) {
          ...
          if (! member.groupType) {
            var user = new User(pickAttributes(member, joinAttributes((opts || {}).attributes || defaultAttributes.user)));
            self.emit(user);
            users.push(user);
            asyncCallback();
          }
  ...
  getUserDistinguishedName.call(self, opts, username, function(err, dn) {
    ...
    getGroupMembershipForDN.call(self, opts, dn, function(err, groups) {
      ...
      _.each(groups, function(group) {
        var result = new Group(pickAttributes(group, joinAttributes((opts || {}).attributes || [], defaultAttributes.group)));
        self.emit(result);
        results.push(result);
      });
  ...
  search.call(self, localOpts, function onFind(err, results) {
    ...
    async.forEach(results, function(item, asyncCallback) {
      if (isGroupResult(item)) {
        var group = new Group(pickAttributes(item, joinAttributes((opts || {}).attributes || [], defaultAttributes.group)));
        ...
        var user = new User(pickAttributes(item, joinAttributes((opts || {}).attributes || [], defaultAttributes.user)));
        result.users.push(user);
        // Also retrieving user group memberships?
        if (includeGroupMembershipFor(opts, 'user')) {
          console.log(opts, user.dn)
        ...
        var other = pickAttributes(item, joinAttributes((opts || {}).attributes || [], defaultAttributes.group || [], defaultAttributes.user || []));
...
ActiveDirectory.prototype.findGroup = function findGroup(opts, groupName, callback) {
  ...
  search.call(self, localOpts, function onSearch(err, results) {
    ...
    var group = new Group(pickAttributes(results[0], joinAttributes((opts || {}).attributes || [], defaultAttributes.group)));
  ...
  search.call(self, localOpts, function onSearch(err, results) {
    ...
    async.forEach(results, function(result, asyncCallback) {
      if (isGroupResult(result)) {
        var group = new Group(pickAttributes(result, joinAttributes((opts || {}).attributes || [], defaultAttributes.user)));
        groups.push(group);
...
ActiveDirectory.prototype.findUser = function findUser(opts, username, includeMembership, callback) {
  ...
  search.call(self, localOpts, function onSearch(err, results) {
    ...
    var user = new User(pickAttributes(results[0], joinAttributes((opts || {}).attributes || [], defaultAttributes.user)));
  ...
  search.call(self, localOpts, function onSearch(err, results) {
    ...
    async.forEach(results, function(result, asyncCallback) {
      if (isUserResult(result)) {
        var user = new User(pickAttributes(result, joinAttributes((opts || {}).attributes || [], defaultAttributes.user)));
        users.push(user);
